...
bool is_p_peak_zc_candidate_exist(const ECGLead& ecg_lead, size_t qrs_id, std::vector<ZeroCrossing> zcs) {
    ...
    size_t zcs_peaks_candidates_begin_index = begin_index + int(float(ZCS_PEAK_SEARCHING_SHIFT) * sampling_rate);
    size_t zcs_peaks_candidates_end_index = end_index;

    bool result = false;

    for (auto &zc : zcs)
        if ((zcs_peaks_candidates_begin_index < zc.index) && (zc.index < zcs_peaks_candidates_end_index)) {
            result = true;
            break;
        }
...
size_t get_p_peak_zc_id(const ECGLead& ecg_lead, size_t qrs_id, std::vector<ZeroCrossing> zcs) {
    ...
    for (size_t candidate_zc_id = 0; candidate_zc_id < zcs.size()-1; ++candidate_zc_id)
    ...
    for (size_t candidate_zc_id = 0; candidate_zc_id<candidates_zcs_ids.size()-1; ++candidate_zc_id) {
        double shift_percentage = zcs[candidate_zc_id].index - zcs_peaks_candidates_point;
...
size_t get_p_flexure_zc_id(const ECGLead& ecg_lead, size_t qrs_id, std::vector<ZeroCrossing> zcs, size_t peak_zc_id) {
    ...
    for (size_t zc_id = 1; zc_id <zcs.size()-1; ++zc_id) {
        if (abs(static_cast<double>(zcs[zc_id - 1].index) - static_cast<double>(zcs[zc_id].index)) < float(FLEXURE_SHIFT) * rr && \
            abs(static_cast<double>(zcs[zc_id + 1].index) - static_cast<double>(zcs[zc_id].index)) < float(FLEXURE_SHIFT) * rr && \
