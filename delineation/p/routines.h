...
int get_window(const ECGLead& ecg_lead, size_t qrs_id) {
    ...
    std::vector<WaveDelineation> cur_qrs_dels_seq = ecg_lead.qrs_dels;
    ...
    if (!t_dels.empty()) {
        ...
        size_t left_diff = cur_qrs_dels_seq[qrs_id].onset_index - t_dels[corr_t_id].offset_index;

        while (left_diff < 0 && corr_t_id > 0) {
            corr_t_id -= 1;
            left_diff = cur_qrs_dels_seq[qrs_id].onset_index - t_dels[corr_t_id].offset_index;
        }
